...
namespace CS1Profiler.TranslationFramework
{
    ...
    public class Translator
    {
        ...
        {
            UnityEngine.Debug.Log($"{CS1Profiler.Core.Constants.LOG_PREFIX} Setting language to " + (currentIndex < 0 ? "system" : languages.Values[currentIndex].uniqueName));
        ...
        private void LoadLanguages()
        {
            ...
            if (!assemblyPath.IsNullOrWhiteSpace())
            {
                ...
                if (Directory.Exists(localePath))
                {
                    ...
                    foreach (string translationFile in translationFiles)
                    {
                        using (StreamReader reader = new StreamReader(translationFile))
                        {
                            ...
                            {
                                UnityEngine.Debug.LogWarning($"{CS1Profiler.Core.Constants.LOG_PREFIX} Couldn't deserialize translation file: " + translationFile);
                            }
                        }
                    }
                }
                ...
                {
                    UnityEngine.Debug.LogWarning($"{CS1Profiler.Core.Constants.LOG_PREFIX} Translations directory not found");
                }
            }
            else
            {
                UnityEngine.Debug.LogWarning($"{CS1Profiler.Core.Constants.LOG_PREFIX} Assembly path was empty");
            }
        }
        ...
        private string GetAssemblyPath()
        {
            ...
            foreach (PluginManager.PluginInfo plugin in plugins)
            {
                try
                {
                    ...
                    if (mods.FirstOrDefault() is CS1Profiler.Mod)
                    {
            ...
            UnityEngine.Debug.LogError($"{CS1Profiler.Core.Constants.LOG_PREFIX} Assembly path not found");
            throw new FileNotFoundException("CS1Profiler: assembly path not found!");
        }
    }
}